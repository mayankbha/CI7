...
class Ion_auth_model extends CI_Model
{
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', username, first_name, last_name, email, id, password, active, last_login')
		...
		if ($query->num_rows() === 1)
		{
			...
			if ($password === TRUE)
			{
				...
				//update login status 
				$this->update_login_status($user->id);
				
				//add login ststus 9oct
				$exist = $this->check_login_status();
				//print_r($exist);die;
				
				if($exist == 0)
				{
					//echo "else-if";print_r($exist);die;
					$this->db->insert('login_status', array('user_id'=>$user->id,'available'=>'1'));
				}else{
					//echo "else-else";print_r($exist);die;
	...
	**/
	 ...
	 * @author Ben Edmunds             
	 **/ 
	public function update_login_status($id)      
	{
		$this->db->update($this->tables['users'], array('login_status' => '1'), array('id' => $id));

		return $this->db->affected_rows() == 1;     
	}
	
	public function update_logout_status($id)    
	{
		//delete login status of current user
		$this->db->delete('login_status', array('user_id'=>$id));
		
		//$this->db->delete('login_users', array('fk_user_id'=>$id));      
		
		$this->db->update($this->tables['users'], array('login_status' => '0'), array('id' => $id));    
    ...
    {                                       
        $this->db->delete('login_users', array('fk_user_id'=>$user_id));                                        
        return TRUE;                   
    }*/
