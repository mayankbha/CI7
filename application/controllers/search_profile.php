...
class Search_profile extends CI_Controller {
	...
	function index()
	{
		...
		//adding match data
			$exist = $this->member_model->find_profile_match(@$this->session->userdata['user_id']=null);
			...
			if(isset($exist)&&$exist>0){
				$old_data=$this->member_model->get_profile_match($this->session->userdata['user_id']=null);
		...
		$this->load->template('search_profile', $this->session->userdata['current_template'],  $this->data, $return = FALSE);	
	}
