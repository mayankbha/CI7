...
/**
 * Description of forum-groups
 *
 * @author nick
 */
class Forum_groups extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model('forumgroup_model');
    }

    public function index()
    {
        $this->load->admin_template('forum-group-list', $this->data, $return = FALSE);
    }
    ...
    function get_all_record()
    {
        ...
        $data = array(
            'currentPage' => 1,
            'totalRows' => $this->forumgroup_model->totat_count(),
            'perPage' => $per_page,
            'sort' => $sort,
            'filter' => $filter,
        ...
        $user = $this->forumgroup_model->get_list(TRUE, $filter,$sort,$current_page, $per_page);
        $data['data'] = $user['data'];
    ...
    public function add($id = 0)
    {
        ...
        //If edit mode
        $this->data['data'] = ($id == 0) ? array() : $this->forumgroup_model->get_single(array('id' => $id), true);
        ...
        if ($this->form_validation->run() == true)
        {
            ...
            //save data
            $insert_id = $this->forumgroup_model->save($save_data);
            ...
            if (is_int($insert_id) or $insert_id == TRUE)
            {
                ...
                redirect(site_url("admins/forum_groups"), 'refresh');
            } else
            {
                $this->session->set_flashdata('message', array(
        ...
        } else
        {
            ...
            $this->load->admin_template('forum-group-form', $this->data);
        }
    }
    ...
    public function delete($id = 0)
    {
        ...
        if ($id != 0)
        {
            $this->forumgroup_model->delete($id);
